...
function renderWorkflowOrder(orderId) {
  ...
  document.getElementById('adminContent').innerHTML = `
    ...
    <div class="section-grid workflow-detail-grid">
      ...
      ${workflowSectionCard('발주', `${purchaseOrders.length}건`, purchaseOrders.length ? purchaseOrders.map(row => `<article class="mini-doc"><strong>${ERP.escapeHtml(row.po_number)}</strong><span>${ERP.escapeHtml(supplierName(row.supplier_id))} · ${ERP.money(row.total_inc, row.currency)} 세후</span><span class="badge">${ERP.escapeHtml(row.status)}</span></article>`).join('') : '<div class="empty">발주 기록이 없습니다.</div>')}
      ...
      ${workflowSectionCard('선적 Invoice', `${invoices.length}건`, invoices.length ? invoices.map(row => `<article class="mini-doc"><strong>${ERP.escapeHtml(row.invoice_number)}</strong><span>${invoiceContractReferences(row.id).map(ERP.escapeHtml).join(', ') || 'Contract 미연결'}</span><span>${ERP.money(row.total, row.currency)}</span></article>`).join('') : '<div class="empty">선적 Invoice가 없습니다.</div>')}
    </div>`;
}
...
function renderShipmentInvoiceForm(orderId = '') {
  ...
  const contact = primaryCustomerContact();
  document.getElementById('adminContent').innerHTML = `<form class="card form-card" onsubmit="saveShipmentInvoice(event)"><div class="section-title"><div><h2>선적 Commercial Invoice</h2><p>여러 Contract 품목을 한 Invoice에 넣고, 남은 수량만 부분 선적할 수 있습니다.</p></div><button type="button" class="btn btn-soft" onclick="${orderId ? `renderWorkflowOrder('${orderId}')` : 'renderWorkflow()'}">돌아가기</button></div><div class="form-grid three"><div class="field"><label>Invoice 번호 *</label><input id="wfInvoiceNumber" required value="CI-${new Date().toISOString().slice(2,10).replaceAll('-','')}-01"></div><div class="field"><label>발행일</label><input id="wfInvoiceDate" type="date" value="${new Date().toISOString().slice(0,10)}"></div><div class="field"><label>선적일</label><input id="wfShipmentDate" type="date" value="${new Date().toISOString().slice(0,10)}"></div><div class="field"><label>통화</label><select id="wfInvoiceCurrency"><option>USD</option><option>CNY</option><option>KRW</option></select></div><div class="field"><label>Buyer 이메일</label><input id="wfInvoiceEmail" type="email" value="${ERP.escapeHtml(contact?.email || '')}"></div><div class="field"><label>Tracking No.</label><input id="wfTrackingNo"></div><div class="field"><label>Freight</label><input id="wfInvoiceFreight" type="number" min="0" step="0.01" value="0"></div><div class="field"><label>Tax</label><input id="wfInvoiceTax" type="number" min="0" step="0.01" value="0"></div><div class="field"><label>Discount</label><input id="wfInvoiceDiscount" type="number" min="0" step="0.01" value="0"></div></div><section class="form-section"><h2>선적할 Contract 품목</h2>${rows.length ? `<div class="table-wrap"><table><thead><tr><th>Contract / 수주</th><th>품목</th><th>계약수량</th><th>기선적</th><th>이번 선적</th><th>단가</th></tr></thead><tbody>${rows.map(row => `<tr data-shipment-contract-item="${row.item.id}" data-order-id="${row.contract.order_id}" data-currency="${row.contract.currency}"><td><strong>${ERP.escapeHtml(row.contract.contract_number)}</strong><br><small>${ERP.escapeHtml(orderNumber(row.contract.order_id))}</small></td><td>${ERP.escapeHtml(row.item.description)}</td><td>${ERP.number(row.item.quantity,4)}</td><td>${ERP.number(row.shipped,4)}</td><td><input class="shipment-qty" type="number" min="0" max="${row.remaining}" step="0.0001" value="0"></td><td>${ERP.money(row.item.unit_price,row.contract.currency)}</td></tr>`).join('')}</tbody></table></div>` : '<div class="empty">선적 가능한 Contract 잔량이 없습니다.</div>'}</section><div class="field"><label>Payment / Shipping terms</label><input id="wfInvoiceTerms" value="Paid before shipment"></div><div class="form-submit"><button class="btn btn-accent" type="submit">Invoice 저장</button></div></form>`;
}
...
async function saveShipmentInvoice(event) {
  ...
  await loadAdminData();
  renderInvoices();
  ERP.toast('Contract 잔량과 연결해 선적 Invoice를 저장했습니다.', 'success');
}
...
function printContract(contractId) {
  ...
  popup.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${ERP.escapeHtml(contract.contract_number)}</title><style>body{font-family:Arial,"Microsoft YaHei",sans-serif;margin:24mm;color:#111}h1{text-align:center;letter-spacing:.08em}.party{display:grid;grid-template-columns:1fr 1fr;gap:24px;font-size:12px;line-height:1.65;margin:22px 0}.party div{border-top:1px solid #333;padding-top:8px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{border:1px solid #222;padding:8px}th{background:#f1f3f5}.num{text-align:right}.total{font-weight:bold}.meta{display:flex;justify-content:space-between;margin:14px 0;font-size:12px}.actions{position:fixed;right:20px;top:20px}@media print{.actions{display:none}body{margin:12mm}}</style></head><body><button class="actions" onclick="print()">PDF / 인쇄</button><h1>SALES CONTRACT</h1><div class="party"><div><strong>SELLER</strong><br>${ERP.escapeHtml(seller.legal_name || seller.display_name || '')}<br>${ERP.escapeHtml(seller.address || '')}</div><div><strong>BUYER</strong><br>${ERP.escapeHtml(buyer.display_name || '')}<br>${ERP.escapeHtml(contact?.contact_name || '')} · ${ERP.escapeHtml(contact?.email || '')}<br>${ERP.escapeHtml(buyer.address || '')}</div></div><div class="meta"><span>No. ${ERP.escapeHtml(contract.contract_number)}</span><span>Date ${ERP.escapeHtml(contract.issue_date)}</span><span>${ERP.escapeHtml(contract.currency)}</span></div><table><thead><tr><th>No.</th><th>Description</th><th>Qty</th><th>Unit</th><th>Unit Price</th><th>Amount</th></tr></thead><tbody>${lines.map((line,index) => `<tr><td>${index+1}</td><td>${ERP.escapeHtml(line.description)}</td><td class="num">${ERP.number(line.quantity,4)}</td><td>${ERP.escapeHtml(line.unit)}</td><td class="num">${ERP.number(line.unit_price,6)}</td><td class="num">${ERP.number(line.amount,2)}</td></tr>`).join('')}<tr><td colspan="5">Subtotal</td><td class="num">${ERP.number(contract.subtotal,2)}</td></tr><tr><td colspan="5">Freight / Tax / Discount</td><td class="num">${ERP.number(Number(contract.freight)+Number(contract.tax)-Number(contract.discount),2)}</td></tr><tr class="total"><td colspan="5">TOTAL ${ERP.escapeHtml(contract.currency)}</td><td class="num">${ERP.number(contract.total,2)}</td></tr></tbody></table><p><strong>Payment Terms:</strong> ${ERP.escapeHtml(contract.payment_terms || '-')}</p><p>${ERP.escapeHtml(contract.notes || '')}</p></body></html>`);
