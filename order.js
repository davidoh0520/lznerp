...
function renderMyInvoices() {
  document.getElementById('orderContent').innerHTML = `<div class="card card-pad">${orderState.invoices.length ? `<div class="table-wrap"><table><thead><tr><th>인보이스</th><th>발행일</th><th>선적일</th><th>금액</th><th>상태</th><th>영문 출력</th></tr></thead><tbody>${orderState.invoices.map(i=>`<tr><td><strong>${i.invoice_number}</strong></td><td>${ERP.date(i.issue_date)}</td><td>${ERP.date(i.shipment_date)}</td><td>${ERP.money(i.total,i.currency)}</td><td><span class="badge ${i.status==='paid'?'good':'warn'}">${ERP.statusLabel(i.status)}</span></td><td><button class="btn btn-small btn-primary" onclick="printCustomerInvoice('${i.id}')">Invoice</button> <button class="btn btn-small btn-soft" onclick="printCustomerPackingList('${i.id}')">Packing List</button></td></tr>`).join('')}</tbody></table></div>` : '<div class="empty">Contract를 만든 뒤 실제 선적 수량으로 Invoice와 Packing List를 작성하세요.</div>'}</div>`;
}
